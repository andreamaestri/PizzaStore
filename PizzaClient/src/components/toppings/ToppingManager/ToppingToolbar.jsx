...
const ToppingToolbar = ({
	...
	const sortOptions = useMemo(
		() => [
			{
				type: SortType.ALPHA_ASC,
				label: "Alphabetical (A-Z)",
				icon: <ArrowUpIcon />,
			},
			{
				type: SortType.ALPHA_DESC,
		...
		() => ({
			startAdornment: (
				<InputAdornment position="start">
					<SearchIcon sx={inputAdornmentStyles} />
				</InputAdornment>
			),
	...
	const createMenuItemClickHandler = useCallback(
		(type) => () => handleSortMenuItemClick(type),
			...
			<Box sx={boxStyles}>
				<Box sx={innerBoxStyles}>
					<Box sx={contentBoxStyles}>
						<Box>
							<Typography variant="h4" component="h2" sx={typographyStyles}>
								Toppings Menu
							</Typography>
							<Typography variant="body1" sx={bodyTypographyStyles}>
								Manage your toppings with ease. Add, edit, or remove items as
								needed.
							</Typography>
						</Box>
						<Box sx={buttonsBoxStyles}>
							...
							{sortButton}
							{/* Refresh Button */}
							<Tooltip title="Refresh List">{refreshButton}</Tooltip>
						</Box>
					</Box>
				</Box>
			</Box>
